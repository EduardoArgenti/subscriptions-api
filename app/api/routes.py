...
from repositories.Plan import create_plan_with_products, get_all_plans, get_plan_by_id, add_products_to_plans, remove_products_from_plans
...
def read_plans_route(background_tasks: BackgroundTasks, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
...
def read_plan_route(background_tasks: BackgroundTasks, plan_id: int, db: Session = Depends(get_db)):
    plan = get_plan_by_id(db, plan_id)
    route = 'GET /plans/{plan_id}'
    if plan is None:
        raise HTTPException(404, f"Plan {plan_id} not found.")
    background_tasks.add_task(save_log, db, route, f"Fetched plan {plan_id}", 200)
    return plan

@router.post("/plans/", response_model=PlanResponse)
def create_plan_route(background_tasks: BackgroundTasks, plan: PlanCreate, db: Session = Depends(get_db)):
...
def add_products_route(background_tasks: BackgroundTasks, plan_id: int, products: List[ProductCreate], db: Session = Depends(get_db)):
...
def remove_products_route(background_tasks: BackgroundTasks, plan_id: int, products_ids: List[int], db: Session = Depends(get_db)):
